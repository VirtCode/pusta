...
use crate::module::installed::build::{BuiltModule, ModuleEnvironment, ModuleInstructions};
use crate::module::installed::neodepend::{ModuleMotivation, Resolver};
...
fn ask(changes: &Vec<(Module, ModuleInstructions, ModuleMotivation, ModifyType)>, previewer: &str) -> bool {
    ...
    for (m, _, _, what) in changes {
        match what {
            ModifyType::Install => { format!("{} ({}-{})", m.name.bold(), m.qualifier.unique(), m.version.dimmed()) }
            ModifyType::Remove => { format!("{} ({}-{})", m.name.bold(), m.qualifier.unique(), "removal".dimmed())  }
            ModifyType::Update => { format!("{} ({}-{})", m.name.bold(), m.qualifier.unique(), m.version.dimmed())  }
        }
    }

    loop {
        let response = prompt("Apply changes or fine grained view? [Y/n/f] ").to_lowercase().trim();
    ...
    fn print_jobs(built: &Vec<BuiltJob>, really: &Vec<bool>, apply: bool) {
        for job in built.iter().zip(really)
            ...
            for change in &job.changes {
                ...
                info!("        action: {}{}{}",
                    ...
                    if job.root { "root".bright_red().bold().deref() } else { "" })
            }
        }
    }
    ...
    for (module, built, motivation, what) in changes {
        ...
        if let Some(old) = &built.old {
            print_jobs(&old.jobs, &built.revert, false);
        }

        if let Some(new) = &built.new {
            print_jobs(&new.jobs, &built.revert, false);
        }
    ...
    loop {
        let response = prompt("Apply changes or preview content? [Y/n/content] ").to_lowercase().trim();

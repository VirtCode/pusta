...
impl Shell {
    ...
    pub fn preview(&self, path: &Path) -> anyhow::Result<()> {
        let command = self.shell_config.file_previewer.replace(FILE_PREVIEW_KEY, &path.canonicalize()?.to_string_lossy().to_string());

        self.run(&command, false, true)
    }
}
